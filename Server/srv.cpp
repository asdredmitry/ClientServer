...
int main(int argc, char *argv[]) {
    ...
    char * nameOfFile = new char [len];
    for(int s = 0; s < len; s++)
        nameOfFile[s] = buf[s + 1];
    memset(buf,0,BUFLEN);
    ...
    else
    {
        ...
        {
            memset(buf,0,BUFLEN);
            counter = fread(buf + 1,1,BUFLEN - 1,file);
            buf[0] = (unsigned char)counter;
            //std :: cout << " I am here " << std :: endl;
            int nbytes = write(s1,buf,BUFLEN);
            if(nbytes < 0)
            {
                perror("cannot send message");
                exit(EXIT_FAILURE);
            }
            //std :: cout << counter << " couter " << std :: endl;
        }
        memset(buf,0,BUFLEN);
        write(s1,buf,BUFLEN);
        fclose(file);
